...
30이라는 숫자는 시스템 한계를 드러냈다. Davison은 이를 알고 있었다. 논문 §6의 Future Work에는 "계층적 맵으로의 확장"이 명시되어 있었다. 그러나 EKF 내부에서 계층적 구조를 만드는 것은 근본적으로 어려웠다. 공분산 행렬이 모든 landmark 간 상관관계를 빠짐없이 담고 있었기 때문이다.

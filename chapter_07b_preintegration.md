...
더 까다로운 쪽은 degenerate motion이다. 순수 직선 이동에서는 global orientation이, 순수 회전에서는 feature 깊이가, 일정 가속도에서는 monocular scale 전체가 관측되지 않는다. 드론이 hover하거나 자동차가 일정 속도로 직진할 때 VIO scale이 흔들리는 이유가 이것이다. 실무자들은 이걸 경험으로 안다. 이륙 순간, 브레이크 순간, 코너 순간에 scale이 "잠긴다".

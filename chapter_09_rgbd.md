...
오프라인 파이프라인들은 스테레오 혹은 structured light 스캐너로 취득한 포인트 클라우드를 시간을 들여 병합했다. 실내 스캔 장비는 수십만 달러였다. 연구실 바깥에서 이 기술을 쓰는 사람은 없었다. SLAM 커뮤니티는 이미 sparse landmark로 충분히 실용적인 결과를 얻고 있었고, dense 재구성은 그래픽스 쪽 문제로 분류되어 있었다.
...
Newcombe, Izadi를 포함한 Microsoft Research Cambridge 팀은 Kinect가 출시된 시점에 이미 GPU 기반 dense SLAM을 준비하고 있었다. 결과가 2011년 ISMAR에서 발표된 [Newcombe et al. 2011. KinectFusion](https://doi.org/10.1109/ISMAR.2011.6092378)이다.
...
> 🔗 **차용.** KinectFusion의 추적 단계는 Besl & McKay(1992) ICP를 직접 계승한다. 고전 로봇공학 문헌의 기법이 GPU 밀도로 재활용됐다.
...
결과는 당시 기준으로 설득력 있었다. 512³ TSDF가 30Hz로 갱신됐고, 실내 방 한 칸을 몇 분 안에 dense mesh로 복원했다. 추적 drift는 ICP가 절대 표면에 수렴하는 구조 덕분에 feature-based 방식보다 훨씬 작았다.
...
핵심 아이디어는 계층적 최적화다. 가장 빠른 층에서는 현재 프레임과 이전 프레임 사이의 dense depth alignment로 초기 포즈를 잡는다. 그 위 층에서는 SIFT feature를 이용한 sparse frame-to-frame alignment로 보정하고, 세 번째 층에서 sliding-window global bundle adjustment가 누적된 프레임들의 포즈를 재최적화한다. Bundle adjustment는 프레임이 누적될수록 과거 포즈도 재추정한다. "retroactive pose correction"이라고 불린 이 방식은 오프라인 SfM 파이프라인이 모든 데이터를 가진 뒤 정합하는 것과 유사한 효과를 온라인으로 달성하려 했다. 갱신된 포즈 시퀀스는 TSDF에 역투영되어 재통합되므로, 추적 오류가 맵에 그대로 누적되지 않는다.

Dai 팀이 TUM RGB-D 벤치마크에서 보고한 수치는 ElasticFusion을 능가했다. 시각적 재구성 품질도 당시 기준에서 오프라인 COLMAP 파이프라인에 근접한다는 평가를 받았다.

> 📜 **예언 vs 실제.** Dai 등은 2017년 BundleFusion §9에서 "GPU의 지속적 발전으로 real-time online global optimization이 조만간 대중화될 것"이라 적었다. GPU 연산력은 예언대로 빠르게 증가했다. 그러나 관심은 dense SLAM이 아니라 NeRF(Neural Radiance Field)로 이동했다. 고품질 실내 재구성의 사실상 표준은 2021년 이후 COLMAP + NeRF 파이프라인이 됐다. BundleFusion이 열려고 했던 경로 자체가 다른 기술로 우회됐다. `[기술변화]`
...
Kinect가 $150이 아니라 $15,000이었다면, 이 흐름은 5년 이상 늦게 시작됐을 것이다. 소비자 시장을 위한 센서가 연구의 속도를 결정했다.

> 📜 **예언 vs 실제.** Newcombe 등은 2011년 KinectFusion §8에서 "scale-up, outdoor 환경, 더 큰 볼륨으로의 확장"을 후속 과제로 명시했다. scale-up과 더 큰 볼륨은 Kintinuous, ElasticFusion, BundleFusion이 차례로 해결했다. outdoor는 다른 결론에 도달했다. IR 구조광의 햇빛 취약성은 RGB-D 기반 dense SLAM의 근본 한계가 됐고, outdoor는 결국 LiDAR의 영역으로 분리됐다. `[기술변화]`

---

## 9.7 "모든 것을 dense로"의 퇴각

2011년부터 2017년 사이 dense RGB-D SLAM은 Visual SLAM의 주된 방향이 될 것처럼 보였다. 실제 전개는 달랐다.

sparse backend는 계속 지배했다. ORB-SLAM2와 VINS-Mono로 대표되는 2015년 이후의 실용 SLAM 시스템들은 dense 맵을 기본으로 삼지 않았다. 이유는 복합적이었다.

메모리 비용이 컸다. 512³ TSDF는 512MB 이상이 필요하다. 모바일 플랫폼이나 임베디드 시스템에서는 감당하기 어렵다. Octree나 해시맵 기반 변형(Voxblox, OctoMap)이 이를 완화하려 했지만, sparse 방식의 효율성과는 격차가 있었다.

실시간 dense 처리는 GPU를 전제했다. 자율주행 차량의 임베디드 프로세서나 드론의 경량 플랫폼에서는 KinectFusion 수준의 dense 파이프라인을 돌리기 어려웠다.

Kinect의 IR depth는 실외에서 작동하지 않았다. 자율주행과 드론 같은 상용화 요구가 큰 분야 대부분이 실외 환경이었다.

그리고 2020년을 전후해 NeRF가 등장했다. 고품질 dense 재구성을 원하는 수요는 NeRF와 3D Gaussian Splatting으로 이동했다. RGB-D SLAM의 위상은 localization과 mapping의 분리 구조 속에서 depth를 활용하는 추적 보조 정도로 축소됐다.

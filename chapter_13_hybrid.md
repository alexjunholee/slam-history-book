...
이름에 들어간 all-pairs가 이 구조의 차별점을 요약한다. 모든 후보 위치를 동시에 고려하고, 고정 해상도에서 flow field를 점진적으로 refinement한다. 기존 coarse-to-fine 방법(PWC-Net 등)과 달리 flow field를 단일 full-resolution으로 유지한 채 correlation pyramid를 lookup한다. RAFT 논문은 당시 공개 최고 결과와 비교해 KITTI의 F1-all 오류를 16%, Sintel final pass의 endpoint error를 30% 상대 감소시켰다고 보고했다.

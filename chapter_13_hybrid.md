...
DeepFactors가 더 중요하게 증명한 것은 원칙이었다. learned representation은 factor graph의 한 노드 유형으로 통합될 수 있다. geometry optimization이 latent space 위에서 작동할 수 있다. Czarnowski가 내린 결론은 하나였다. "SLAM을 end-to-end로 교체하는 것"이 아니라 "SLAM의 일부를 학습 가능한 모듈로 교체하는 것"이 현실적 경로라는 것.

이 원칙은 2021년 Princeton에서 전혀 다른 방식으로 터져 나왔다.
...
12장의 순수 end-to-end 접근이 왜 실패했는지 돌아보면 DROID-SLAM이 왜 달랐는지 드러난다. PoseNet은 geometry constraint 없이 pose를 직접 회귀했다. 일반화에 실패했다. Teed와 Deng은 역할을 나눴다. frontend에서는 학습이 dense correspondence를 추정하고, backend에서는 BA가 geometry 제약을 강제한다. 학습이 잘 하는 일(feature 추출, dense matching)과 geometry optimization이 잘 하는 일(consistency enforcement, uncertainty 전파)을 각자의 영역에 배치했다.

...
2020년, Jan Czarnowski도 Davison 지도 아래 Imperial Dyson Robotics Lab에서 [Czarnowski et al. 2020. DeepFactors](https://doi.org/10.1109/LRA.2020.2969036)를 발표했다. 목표는 CodeSLAM의 아이디어를 실제 SLAM 파이프라인으로 가져오는 것이었다.
...
더 중요한 것은 DeepFactors가 증명한 원칙이었다. learned representation은 factor graph의 한 노드 유형으로 통합될 수 있다. geometry optimization이 latent space 위에서 작동할 수 있다. 결론은 하나였다. "SLAM을 end-to-end로 교체하는 것"이 아니라 "SLAM의 일부를 학습 가능한 모듈로 교체하는 것"이 현실적 경로라는 것.

이 원칙은 2021년 Princeton에서 전혀 다른 방식으로 폭발했다.
...
"all-pairs"라는 단어가 핵심이다. 특정 이웃 픽셀만 보는 것이 아니라 모든 후보 위치를 동시에 고려한다. 반복(recurrent) 방식으로 고정 해상도에서 flow field를 점진적으로 refinement한다. 기존 coarse-to-fine 방법(PWC-Net 등)과 달리 flow field를 단일 full-resolution으로 유지한 채 correlation pyramid를 lookup하는 것이 RAFT의 핵심 기여다. KITTI, Sintel, FlyingThings3D에서 기존 방법을 5%-15% 앞섰다.
...
아키텍처를 따라가면 hybrid 설계의 의도가 명확하다.

Frontend는 RAFT와 동일한 구조다. CNN encoder가 feature map을 추출하고, all-pairs correlation volume이 구성되고, GRU update operator가 dense flow를 반복 추정한다. 차이는 한 쌍의 이미지 사이 flow가 아니라 keyframe graph의 모든 edge에서 동시에 flow를 추정한다는 것이다.
...
12장에서 다룬 순수 end-to-end 접근의 실패를 돌아보면 왜 성공했는지 답이 보인다. PoseNet은 geometry constraint 없이 pose를 직접 회귀했다. 일반화에 실패했다. DROID-SLAM은 다르다. frontend에서는 learning이 dense correspondence를 추정하고, backend에서는 BA가 geometry 제약을 강제한다. learning이 잘 하는 일(feature 추출, dense matching)과 geometry optimization이 잘 하는 일(consistency enforcement, uncertainty 전파)을 각자의 영역에서 수행한다.

인간이 설계한 feature를 학습된 feature로 교체했다. 그러나 optimization structure는 남겼다. 이것이 2021년의 hybrid가 2015년의 end-to-end와 다른 이유다.

---

## 13.5 Imperial Dyson Lab 계보도

CodeSLAM에서 DROID-SLAM까지의 흐름을 이해하려면 Imperial Dyson Robotics Lab의 인적 계보를 따라가는 것이 도움이 된다.

Andrew Davison은 2002년 MonoSLAM 이후 20년 동안 Imperial에서 SLAM 연구를 이끌었다. 그의 제자·협력자 중 상당수가 현대 visual SLAM의 분기점을 만들었다.
...
이 계보는 "학파"라는 단어가 과장이 아닌 경우 중 하나다. 동일한 factor graph + uncertainty 철학이 단안 sparse에서 dense latent로, dense latent에서 implicit representation으로 변형되며 이어진다. Davison 자신은 2020년 FutureMapping 논문 시리즈에서 "universal scene representation"이라는 방향을 공개적으로 선언했다. CodeSLAM과 DeepFactors는 그 방향의 초기 실험이었다.
...
DPV-SLAM(2024)은 DROID-SLAM의 frontend feature를 DINOv2 기반 foundation visual feature로 교체했다. generalization이 향상된다는 보고가 있다.

공통점이 있다. tracking은 DROID 또는 유사한 recurrent dense flow 방식을 유지하고, map representation 또는 feature를 교체한다. 2021년 DROID가 제공한 learned frontend + classical backend 프레임워크가 기반 구조가 되었다.
...
hybrid 계보가 도달한 곳은 결국 하나의 물음으로 수렴한다. map representation을 점, 선, 평면으로 유지해야 하는가. DROID-SLAM의 inverse depth map은 2021년 기준 최선의 dense representation이었다. 그러나 2020년, NeRF(Neural Radiance Field)가 전혀 다른 가능성을 제시했다. 장면을 포인트나 메시가 아니라 연속 함수로 표현하면 어떤가. 렌더링이 미분 가능하다면 photometric consistency를 새로운 방식으로 강제할 수 있다.

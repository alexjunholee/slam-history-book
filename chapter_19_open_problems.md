...
18개 챕터의 마지막 절에는 동일한 표시가 남아 있다. 아직 열린 것들. 창작이 아니라 수확이다. 각 챕터가 풀었다고 선언한 자리 바로 옆에 꽂아둔 깃발들을 한 자리에 펼치는 것이다.
...
Learned descriptor는 훈련 도메인에선 ORB를 능가하지만 underwater·thermal·low-light에서 일관성이 없고, 2026년에도 우열 합의가 없다 (Ch.2 §2.7 참조). Ch.5가 기록한 저조도·동적 추적 실패, 2007년 PTAM이 "Small AR Workspaces"로 스스로 범위를 제한한 이유가 대부분의 feature-based SLAM에 지금도 암묵 가정으로 남아 있다 (Ch.5 §🧭 참조).
...
Foundation 3D에서는 이 문제가 loop closure 재정의로 이어진다. DUSt3R 계열에서 pointmap 기반 교정 propagate는 MASt3R-SLAM이 기존 방식으로 처리하지만 원리적 해법인지는 불확실하다 (Ch.16 §🧭 참조). 자율주행·의료 로봇에서 calibrated uncertainty가 필수인데 그 수준 시스템은 드물다.
...
Event camera는 모달리티가 새로울 때 알고리즘이 얼마나 뒤따르는지를 보여준다. 2022년 이후 상업 고해상도 event camera가 보급되었지만 frame 기반 pipeline과의 통합, event representation, real-world benchmark가 동시 진행 중이다 (Ch.18 §🧭 참조). Kinect가 2010년 출시되고 KinectFusion이 1년 뒤 나온 것과 같은 순서다.

이 책이 범위 밖으로 둔 모달리티가 있다. **4D imaging radar**와 **legged/proprioceptive SLAM**이다. Radar는 안개·강우에서 카메라·LiDAR가 동시 실패하는 조건을 보완하는 유일한 상용 센서로, Oxford Radar RobotCar(2019), NuScenes, 2023년 이후 4D imaging radar(Arbe, Mobileye)가 자율주행 주류에 진입했다. Legged SLAM은 ANYmal·Spot·Unitree의 2020년대 실외 배포와 함께 kinematic·contact prior 융합의 별도 계보를 열었다. 둘 다 visual·LiDAR·foundation 3D와 다른 원류·벤치마크를 가지며, 각자의 역사서가 필요한 크기다.
...
Dennard scaling 붕괴로 단일 코어 clock speed가 2000년대 중반 4GHz에서 멈췄고 *"this has stopped being true"* (Handbook Ch.18, p.528)인 동시에, 착용형 Spatial AI의 제약은 안경 한 짝—65g, <1W—으로 남아 있다. 이 간극이 **heterogeneous·specialized·parallel** 아키텍처로 분야를 밀어넣는다.

구체 실리콘 사례가 2020년대 중반에 모였다. [Apple Vision Pro R1](https://www.apple.com/apple-vision-pro/specs/)(2023)은 센서 데이터 12 ms 처리 전용 칩, [Meta ARIA Gen 2](https://www.projectaria.com/ariagen2/)(2024)는 "ultra low power and on-device machine perception" custom silicon, [Graphcore IPU](https://www.graphcore.ai/products/ipu)는 수천 코어가 로컬 메모리와 메시지 패싱, Manchester [SCAMP5](https://personalpages.manchester.ac.uk/staff/p.dudek/papers/carey-iscas2013.pdf)는 256×256 per-pixel in-plane processing 1.2W, [SpiNNaker](https://apt.cs.manchester.ac.uk/projects/SpiNNaker/)는 ARM 코어 최대 100만 개의 neuromorphic 구조. 각자 다른 graph topology를 요구하고, 어느 실리콘에 어떻게 매핑할지에 대한 체계적 이론은 아직 없다.

...
2003년 Davison이 웹캠 한 대로 실시간 3D 추적을 증명하던 무렵, 옥스퍼드 모바일 로보틱스 그룹에서는 다른 질문이 제기되고 있었다. "로봇이 이전에 지나간 장소를 어떻게 알아보는가?" VO(visual odometry)가 누적 drift에 시달리는 한, 이 질문의 답이 없으면 어떤 SLAM 시스템도 루프를 닫을 수 없었다. Place recognition은 Visual SLAM의 나머지 구성 요소들과 평행하게, 그러나 독자적인 계보로 2000년대 내내 발전했다. FAB-MAP은 Josef Sivic의 BoW 아이디어를 로봇 공간으로 이식했고, DBoW2는 그것을 실용화했으며, NetVLAD는 학습으로 끊어냈다. 2023년 AnyLoc는 foundation model의 feature를 그대로 가져왔다.
...
2000년대 초 컴퓨터 비전에서 이 문제를 먼저 건드린 것은 Sivic과 Zisserman이었다. 2003년 ICCV에서 발표된 "Video Google"은 문서 검색의 TF-IDF를 이미지에 적용했다. SIFT 기술자를 k-means로 군집화해 "visual word"를 만들고, 이미지를 그 단어들의 빈도 벡터로 표현했다. 검색은 inverted index를 통해 O(1)에 가까워졌다. 이 접근이 place recognition 연구자들에게 직접 영향을 미쳤다.
...
또한 FAB-MAP은 현재 위치가 데이터베이스에 없는 새 장소일 가능성을 명시적으로 다룬다. 이 "new place" 가설이 포함됨으로써 false positive를 억제할 수 있었다. Loop closure에서 false positive는 catastrophic failure로 이어지기 때문에, 이 특성은 실용적으로 중요했다.
...
DBoW2가 중요한 것은 알고리즘의 독창성보다 실용성에 있다. 오픈소스로 공개된 이 라이브러리는 ORB-SLAM(2015)의 loop closure 모듈로 채택되었고, 이후 ORB-SLAM2·ORB-SLAM3까지 같은 DBoW2를 사용했다. 사실상 2015-2020년대 중반까지 SLAM 커뮤니티의 place recognition 표준 구성 요소가 된 것이다.
...
Relja Arandjelović, Petr Gronat, Akihiko Torii, Tomáš Pajdla, Josef Sivic는 2016년 CVPR에서 [NetVLAD: CNN Architecture for Weakly Supervised Place Recognition](https://doi.org/10.1109/CVPR.2016.572)을 발표했다. 저자 중 Sivic은 "Video Google"의 Sivic이다. BoW 아이디어의 시작점에 있던 사람이 14년 후 그 한계를 넘는 논문을 함께 썼다.
...
NetVLAD 이후 VPR(Visual Place Recognition) 연구는 일반화 성능 개선을 주제로 분기했다.

2021년 Hausler et al.은 Patch-NetVLAD를 제안했다. Global descriptor 하나로 장소를 판단하는 NetVLAD 대신, 이미지를 패치로 분할해 각 패치의 NetVLAD 표현을 공간적으로 결합하는 방식이다. Tokyo 24/7에서 NetVLAD 대비 Recall@1을 약 10% 포인트 개선했다. 그러나 패치 단위 처리로 인해 추론 비용이 더 증가했다.

2023년 Ali-bey et al.의 MixVPR는 Transformer-style feature mixing으로 Global feature를 생성하는 접근이었다. 경량화와 성능의 균형을 맞추는 데 초점을 두었다. 이 시기 VPR 논문들은 공통적으로 Mapillary Street Level Sequences(MSLS)와 Nordland 같은 계절 변화 데이터셋을 벤치마크로 삼았고, 극단적 조명·계절 조건이 공통의 장벽으로 부각되었다.

2023년 Keetha et al.의 [AnyLoc: Towards Universal Visual Place Recognition](https://arxiv.org/abs/2308.00688)은 다른 방향을 택했다. DINOv2 기반의 self-supervised feature를 추가 fine-tuning 없이 그대로 place recognition에 사용하는 것이다.
...
Place recognition 연구는 2000년대 초부터 SLAM의 나머지 구성 요소와 평행하게 발전했다. ORB-SLAM이 DBoW2를 내장했다는 의미에서 연결은 있었지만, place recognition 모듈은 mapping과 tracking으로부터 격리된 블랙박스였다. 입력은 이미지, 출력은 루프 후보 ID였다.

2024-2025년에는 이 경계가 흐려지기 시작했다. EigenPlaces(2023, Berton et al.)와 Salad(2023, Izquierdo et al.)는 place recognition descriptor가 metric localization에 직접 사용될 수 있는 방향을 탐구했다. 단순히 "어디서 본 장소"를 찾는 것이 아니라 정확한 6-DoF pose를 place recognition 표현에서 바로 추정하려는 시도였다.

한편 Sphinx(2024)는 Gaussian map 표현과 place recognition을 결합했다. 이 방향은 3DGS(3D Gaussian Splatting)가 지도 표현으로 부상한 흐름과 맞물린다.

> 📜 **예언 vs 실제.** Cummins와 Newman은 2011년 FAB-MAP 2.0 §6에서 "1,000 km 규모 place recognition이 목표"라고 적었다. 2015년경 DBoW2와 대형 vocabulary를 사용한 도시 규모 루프 클로저 실험들이 이 규모를 실현했다. 규모 문제는 예언대로 해결되었다. 그러나 Cummins와 Newman이 묘사한 실패 모드, 즉 계절·조명 변화에 취약한 vocabulary 기반 표현의 한계는 해결되지 않았고, 그 문제를 넘어선 것은 deep learning 기반 방법들이었다. `[기술변화]`

> 📜 **예언 vs 실제.** Arandjelović et al.은 2016년 NetVLAD §6에서 "end-to-end VPR의 일반화는 아직 열린 도전"이라 했다. 이후 7년간 수십 편의 VPR 논문들이 외관 조건 일반화를 목표로 발표되었다. 2023년 AnyLoc은 fine-tuning 없는 foundation model feature로 다환경 단일 모델의 가능성을 보였다. 완전한 해결이라기보다는 "다루기 가능한" 수준으로 이전했다고 보는 것이 정확하다. `[진행형]`
...
*인식 가능한 장소 표현의 프라이버시.* VPR 시스템이 저장하는 장소 표현은 복원 공격에 의해 원본 이미지나 3D 구조를 재구성하는 데 이용될 수 있다. 특히 상업 로봇이 가정·병원·사무실 실내를 매핑할 때 이 문제가 현실적이 된다. 성능 저하 없이 프라이버시를 보장하는 장소 표현 방식은 아직 해결책이 없다.

---

3부(성숙기)의 세 계보는 이렇게 막을 내린다. ORB-SLAM은 feature-based 파이프라인을 표준화했고, DSO는 그 반대편에서 photometric 이론을 완성했으며, KinectFusion 계열은 dense mapping의 가능성과 한계를 함께 드러냈다. Place recognition은 이 세 계보와 달리 SLAM 내부에서 자란 것이 아니었다. 컴퓨터 비전의 이미지 검색 문제에서 자라나, SLAM이 필요로 할 때 공급자 역할을 맡았다. 그 독립성이 오히려 장기적 유연성이 되었다. deep learning 물결이 닥쳤을 때, place recognition은 기존 파이프라인보다 빠르게 새 도구를 흡수할 수 있었다.

2023년 AnyLoc이 등장했을 때, place recognition 연구자들이 14년 전 BoW를 처음 제안했던 Sivic의 이름이 논문 감사의 글이 아니라 참고문헌에 있었다. 같은 사람이 문을 열고, 다른 사람들이 그 문을 지나쳤으며, 다시 Sivic이 공동저자로 그 문을 닫았다. 그것으로 3부가 끝난다.

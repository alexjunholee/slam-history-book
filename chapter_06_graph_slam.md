...
당시 Lu-Milios의 해법은 모든 포즈를 동시에 푸는 배치(batch) 선형 시스템이었다. 스캔 수가 늘어나면 선형 시스템의 크기도 함께 커진다. 그래서 현장 적용보다는 개념 증명에 가까웠다. 그러나 두 가지를 확실히 보여주었다. 전역 일관성은 달성 가능하다. 그리고 그 도구는 필터가 아닌 최적화다. 같은 시기 Gutmann-Konolige는 증분성에, Folkesson-Christensen은 데이터 연관 강건성에, Thrun 그룹은 대규모 실환경 적용에 각자 방점을 찍으며 같은 결론의 각도를 다르게 깎고 있었다.

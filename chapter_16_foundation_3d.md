...
Naver Labs Europe의 Philippe Weinzaepfel과 Jerome Revaud는 2022년 CroCo를 발표하면서, 두 이미지가 같은 장면을 찍었다는 사실을 단서 삼아 visual representation을 학습하는 cross-view self-supervised pretraining 방식을 제안했다. 그것은 feature learning 논문처럼 보였다. 1년 뒤 같은 팀이 CroCo의 구조 위에서 calibration 없이 pointmap을 직접 출력하는 시스템을 만들었을 때, DUSt3R는 단순한 feature learning이 아니라 multi-view geometry 전체를 재정의하는 논문이 되었다. Naver Labs Europe에서 시작한 계보가 Oxford의 VGG 그룹으로 이어지며, 2026년 현재 "SfM이 무엇인가"라는 질문 자체를 다시 쓰고 있다.
...
Leroy의 논문 마지막 절은 "ground-truth calibration이 없어도 된다는 특성이 모든 downstream 태스크에 영향을 줄 것"이라 적었다. 2026년 현재 COLMAP 기반 SfM 파이프라인에 DUSt3R나 MASt3R를 삽입하는 것이 실험 설정에서 표준화되고 있다.

> 📜 **예언 vs 실제.** Wang 2023 DUSt3R §6 "Limitations and Future Work"는 "대규모 sequence, 실시간 처리, video stream 지원이 다음 과제"라 명시했다. Spann3R는 2024년 8월, MASt3R-SLAM은 2024년 말 나왔다. 두 후속 작업이 각각 sequential extension과 SLAM 통합 문제에 6-12개월 내에 응답했다. 이 속도 자체가 이 분야의 이상한 점이다. 저자들이 "미래"라 부른 것을 다른 팀이 이미 진행 중이었다는 뜻이기도 하다. `[적중+진행형]`
...
Spann3R는 sequential 처리를 가능하게 했다. 그러나 pair-wise pointmap + global alignment라는 DUSt3R의 기본 골격은 그대로였다. Oxford의 VGG 그룹이 다시 들어온다. Kaiqian Xu, Andrea Vedaldi, Philip Torr — Spann3R를 낸 팀과 같은 곳이다 — 가 2025년 초 DUSt3R의 논리를 끝까지 밀어붙였다. 두 이미지가 아니라 임의의 다중 이미지를 동시에 입력받아, 카메라 포즈와 깊이, 점군을 한 번의 forward pass로 출력한다.

[Kaiqian Xu et al. 2025. VGGT: Visual Geometry Grounded deep structured Transformers](https://arxiv.org/abs/2503.11651)는 DUSt3R의 pair-wise 처리를 진정한 multi-view joint inference로 바꿨다. DUSt3R에서 N장의 이미지를 처리하려면 N(N-1)/2쌍의 pointmap을 구한 뒤 global alignment를 풀어야 한다. VGGT는 N장을 한꺼번에 transformer에 통과시킨다. attention이 모든 이미지 쌍 사이의 관계를 동시에 처리한다.
...
> 📜 **예언 vs 실제.** Leroy 2024 MASt3R 논문은 명시적이다: "ground-truth calibration 없이도 된다는 특성이 모든 downstream에 영향을 줄 것이며, 특히 incremental SfM 파이프라인의 restructuring이 예상된다." 2026년 현재 여러 photogrammetry 소프트웨어가 DUSt3R/MASt3R를 initialization 단계로 채택하는 것을 평가 중이다. 전면 교체보다는 hybrid 삽입의 형태가 보인다. `[진행형]`

Naver Labs Europe이라는 한 연구소가 CroCo(2022) → DUSt3R(2023) → MASt3R(2024) → MASt3R-SLAM(2024)의 단계를 2년 내에 밟았다. 이 속도는 특이하다. 한 팀이 pretraining 방법론부터 SLAM 시스템까지의 전 스택을 연속해서 발표했다. Google Brain, DeepMind, Meta AI처럼 규모가 큰 곳이 아니라 Naver Labs Europe이었다. Weinzaepfel·Revaud·Leroy를 중심으로 한 소규모 팀의 집중적 투자가 만든 결과다.

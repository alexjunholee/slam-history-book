...
Naver Labs Europe의 Philippe Weinzaepfel과 Jerome Revaud는 2022년 CroCo를 발표하면서, 두 이미지가 같은 장면을 찍었다는 사실을 단서 삼아 visual representation을 학습하는 cross-view self-supervised pretraining 방식을 제안했다. 그것은 feature learning 논문처럼 보였다. 1년 뒤 같은 팀이 CroCo의 구조 위에서 calibration 없이 pointmap을 직접 출력하는 시스템을 만들었을 때, DUSt3R는 단순한 feature learning이 아니라 multi-view geometry 전체를 재정의하는 논문이 되었다. 이 챕터의 계보는 Naver Labs Europe에서 시작해, Oxford의 VGG 그룹으로 이어지며, 2026년 현재 "SfM이 무엇인가"라는 질문 자체를 다시 쓰고 있다.

---

## 16.1 질문을 바꾼다는 것

2013년부터 10년간 3D 재건은 동일한 절차를 따랐다. 특징점을 찾고, 매칭하고, 카메라 내부 파라미터와 외부 파라미터를 추정하고, triangulation으로 점군을 만들고, bundle adjustment로 전체를 정제한다. COLMAP(Schönberger & Frahm, 2016)이 이 파이프라인의 가장 완성된 형태였다. 오차는 줄었지만 절차의 구조는 바뀌지 않았다.

[Shuzhe Wang et al. 2023. DUSt3R: Geometric 3D Vision Done Right](https://arxiv.org/abs/2312.14132)는 이 절차를 우회한다. 두 이미지를 입력으로 받아 각 픽셀에 대한 3D 좌표를 직접 출력한다. 내부 파라미터(focal length, principal point)를 요구하지 않는다. pointmap이라 불리는 이 출력은, 이미지 좌표계가 아닌 공통된 3D 공간에서의 좌표다. 카메라가 어떤 렌즈를 달고 있는지 몰라도 된다.

왜 가능한가. DUSt3R의 transformer는 CroCo에서 물려받은 encoder-decoder 구조를 쓴다. 각 이미지는 독립적으로 encoding된 뒤, decoder에서 cross-attention을 통해 다른 이미지의 encoder 출력을 참조한다. self-attention이 단일 이미지 내 픽셀 관계를 처리한다면, cross-attention은 두 이미지 사이의 대응을 암묵적으로 학습한다. 어느 픽셀이 어느 픽셀과 같은 3D 점을 보는지, 그것을 규칙으로 코딩하는 대신 대규모 데이터에서 패턴으로 흡수한다. DUSt3R의 훈련 데이터는 MegaDepth, ScanNet, ARKitScenes, BlendedMVS 등 수백만 장의 이미지 쌍이다. ground truth는 COLMAP이 만들었다. 고전 SfM이 학습 시대의 ground truth를 공급한다는 역전이 여기서 일어난다.
...
두 이미지에서 pointmap 한 쌍을 얻으면, 카메라 포즈는 이 pointmap들 사이의 rigid alignment로 구한다. Procrustes alignment의 일반화다. pose estimation이 별도 단계가 아니라 pointmap의 파생물이 된다.
...
MASt3R 공개 이후 수개월 내에 SLAM 커뮤니티에서 SuperPoint+SuperGlue 조합을 MASt3R로 교체하는 실험이 여러 그룹에서 보고되었다. 2024년 말 Naver Labs Europe이 MASt3R-SLAM을 공개했을 때, 이 시스템은 MASt3R의 매칭을 frontend로, global pointmap alignment를 backend로 사용했다. 고전적 SLAM 아키텍처의 모양은 유지하되 내부 부품이 거의 전부 교체된 형태다.

Leroy의 논문 마지막 절은 "ground-truth calibration을 필요로 하지 않는다는 특성이 모든 downstream 태스크에 영향을 줄 것"이라 적었다. 2026년 현재 COLMAP 기반 SfM 파이프라인에 DUSt3R나 MASt3R를 삽입하는 것이 실험 설정에서 표준화되고 있다.

> 📜 **예언 vs 실제.** Wang 2023 DUSt3R §6 "Limitations and Future Work"는 "대규모 sequence, 실시간 처리, video stream 지원이 다음 과제"라 명시했다. Spann3R는 2024년 8월, MASt3R-SLAM은 2024년 말 나왔다. 두 후속 작업이 각각 sequential extension과 SLAM 통합 문제에 6-12개월 내에 응답했다. 이 속도 자체가 이 분야의 이상한 점이다. 저자들이 "미래"라 부른 것이 이미 다른 팀에 의해 진행 중이었다는 뜻이기도 하다. `[적중+진행형]`

---

## 16.3 순서를 갖는다: Spann3R
...
[Hengyi Wang et al. 2024. Spann3R: Spatio-temporal Attention for 3D Reconstruction](https://arxiv.org/abs/2408.16061)는 DUSt3R의 구조를 sequential 처리에 맞게 고친다. 핵심 아이디어는 spatial memory다. 이미 처리한 프레임들의 정보를 memory bank에 저장하고, 새 프레임이 들어올 때 이 메모리에 cross-attention을 수행한다. 새 이미지의 각 픽셀이 과거 프레임의 어떤 정보와 연관되는지 attention이 결정한다.

> 🔗 **차용.** Spann3R의 spatial memory 메커니즘은 concept 면에서 codebook-based memory나 cross-attention memory(Jain et al., Perceiver IO)와 유사하지만, 핵심은 DINOv2([Oquab et al. 2023](https://arxiv.org/abs/2304.07193)) feature를 memory의 키로 사용한다는 점이다. DINOv2가 제공하는 풍부한 visual feature가 어떤 과거 정보를 찾아야 할지를 안정적으로 안내한다. foundation visual feature의 3D 재건으로의 확장이 이 경로로 이루어졌다.

Spann3R는 calibrated 카메라를 필요로 하지 않는다는 DUSt3R의 특성을 유지한다. 순차 이미지가 들어올 때마다 현재까지의 지도를 점진적으로 갱신한다. 완전한 실시간은 아니지만, DUSt3R의 일괄 처리 방식보다 SLAM 적용에 한 발 더 가깝다.

---

## 16.4 끝까지 간다: VGGT

Spann3R는 sequential 처리를 가능하게 했다. 그러나 pair-wise pointmap + global alignment라는 DUSt3R의 기본 골격은 그대로였다. Oxford의 VGG 그룹이 다시 들어온다. Spann3R와 같은 그룹에서 Kaiqian Xu, Andrea Vedaldi, Philip Torr가 DUSt3R의 논리를 더 밀었다. 두 이미지가 아니라 임의의 다중 이미지를 동시에 입력받아, 카메라 포즈와 깊이, 점군을 한 번의 forward pass로 출력한다.

VGGT(Visual Geometry Grounded deep structured Transformers)는 2024-2025년에 공개되었다. 핵심 차이는 DUSt3R의 pair-wise 처리를 진정한 multi-view joint inference로 대체한다는 점이다. DUSt3R에서 N장의 이미지를 처리하려면 N(N-1)/2쌍의 pointmap을 구한 뒤 global alignment를 풀어야 한다. VGGT는 N장을 한꺼번에 transformer에 통과시킨다. attention은 모든 이미지 쌍 사이의 관계를 동시에 처리한다.
...
> 📜 **예언 vs 실제.** Leroy 2024 MASt3R 논문은 명시적이다: "ground-truth calibration을 필요로 하지 않는다는 특성이 모든 downstream에 영향을 줄 것이며, 특히 incremental SfM 파이프라인의 restructuring이 예상된다." 2026년 현재 여러 photogrammetry 소프트웨어가 DUSt3R/MASt3R를 initialization 단계로 채택하는 것을 평가 중이다. 전면 교체보다는 hybrid 삽입의 형태가 보인다. `[진행형]`

Naver Labs Europe이라는 한 연구소가 CroCo(2022) → DUSt3R(2023) → MASt3R(2024) → MASt3R-SLAM(2024)의 단계를 2년 내에 밟았다. 이 속도는 특이하다. 한 팀이 pretraining 방법론부터 SLAM 시스템까지의 전 스택을 연속해서 발표했다. Google Brain, DeepMind, Meta AI처럼 규모가 큰 곳이 아닌 Naver Labs Europe이라는 사실도 주목할 만하다. Weinzaepfel·Revaud·Leroy를 중심으로 한 소규모 팀의 집중적 투자가 만든 결과다.
...
이 관찰은 5부 전체에 걸쳐 반복되는 패턴과 일치한다. NeRF-SLAM이 NeRF를 map 표현으로 채택하면서도 keyframe 기반 tracking을 유지했다. 3DGS-SLAM이 Gaussian을 채택하면서도 loop closure를 classical 방식으로 했다. 표현(representation)이 바뀔 때 system 구조는 보수적으로 재사용되는 경향이 있다.

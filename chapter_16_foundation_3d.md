...
Leroy의 논문 마지막 절은 ground-truth calibration 없이도 된다는 특성이 모든 downstream 태스크에 영향을 줄 것이라 주장했다. 2026년 현재 COLMAP 기반 SfM 파이프라인에 DUSt3R나 MASt3R를 삽입하는 것이 실험 설정에서 표준화되고 있다.

> 📜 **예언 vs 실제.** Wang 2023 DUSt3R 논문은 한계 절에서 대규모 sequence 처리, 실시간 구동, video stream 지원을 다음 과제로 꼽았다. Spann3R는 2024년 8월, MASt3R-SLAM은 2024년 말 나왔다. 두 후속 작업이 각각 sequential extension과 SLAM 통합 문제에 6-12개월 내에 응답했다. 이 속도 자체가 이 분야의 이상한 점이다. 저자들이 "미래"라 부른 것을 다른 팀이 이미 진행 중이었다는 뜻이기도 하다. `[적중+진행형]`
...
> 📜 **예언 vs 실제.** Leroy 2024 MASt3R 논문은 결론부에서 ground-truth calibration이 불필요한 특성이 모든 downstream에 영향을 줄 것이며, incremental SfM 파이프라인의 구조적 재편이 예상된다고 주장했다. 2026년 현재 여러 photogrammetry 소프트웨어가 DUSt3R/MASt3R를 initialization 단계로 채택하는 것을 평가 중이다. 전면 교체보다는 hybrid 삽입의 형태가 보인다. `[진행형]`

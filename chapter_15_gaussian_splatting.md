...
Volume rendering 적분을 수치적으로 근사하는 NeRF와 달리, 3DGS는 GPU 래스터화(rasterization) 파이프라인에 직접 올라간다. 타일 기반 래스터라이저는 forward pass와 backward pass 모두 커스텀 CUDA 커널로 구현했다. RTX 3090 한 장에서 30fps 이상. NICE-SLAM이 같은 카드에서 0.1fps를 내던 것과 비교하면 300배다.
...
> 📜 **예언 vs 실제.** Keetha는 SplaTAM(2024) 한계 절에서 monocular 카메라, outdoor 환경, 더 큰 스케일을 명시적 과제로 제시했다. monocular 확장은 같은 해 Matsuki의 MonoGS(2024 CVPR)가 해결했다. outdoor와 large-scale은 2024-2025년 현재 진행 중이다. `[적중+진행형]`
...
GS-SLAM 계열이 풀어야 할 다음 문제는 속도였다. GS-SLAM과 SplaTAM은 실시간이라 부르기 어려웠다. Zhejiang University의 Peng Zhen 연구팀이 2024년 발표한 RTG-SLAM은 명시적으로 실시간을 목표로 삼았다.
...
> 📜 **예언 vs 실제.** Kerbl et al.은 3DGS(2023) 한계 절에서 정적 장면 한정, dynamic scene 처리, editing 지원, training 안정화를 미해결 과제로 지적했다. dynamic 방향은 2024년 4DGS(Wu et al.), Deformable 3DGS 등이 본격 탐구를 시작했고, editing은 Dreamgaussian·GaussianEditor가 대응했다. training 안정화(densification heuristics의 민감성)는 2025년 현재도 진행 중이다. `[진행형]`
...
Semantic 통합. Gaussian에 semantic label을 붙이는 시도(LangSplat, LERF 등)가 2023-2024년에 나왔다. 그러나 SLAM 파이프라인에서 semantic Gaussian을 실시간으로 갱신하면서 tracking 품질을 동시에 유지하는 방법은 아직 없다. semantic과 geometry를 공동 최적화할 때 발생하는 interference를 어떻게 처리할 것인가가 핵심 문제다.

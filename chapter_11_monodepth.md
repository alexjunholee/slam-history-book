...
> 📜 **예언 vs 실제.** MiDaS(2020)는 scale-and-shift invariant loss로 절대 스케일을 포기하고 상대 깊이에만 집중하는 우회를 택했다. 그 선택 뒤에는 카메라 파라미터 없이 metric 복원을 하기는 본질적으로 어렵다는 판단이 있었다. 2024년 Depth Anything v2와 Metric3D v2가 camera intrinsic을 입력으로 받는 방식으로 이 방향을 직접 공략했고, in-the-wild metric이 실용 수준에 가까워졌으나 완전한 카메라 독립은 아직 아니다. `[진행형]`

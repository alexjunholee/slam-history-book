...
둘째, 조명 변화다. Engel은 DSO 논문 §7.3에서 photometric gradient가 부족한 환경에 들어서면 추적 실패율이 급격히 오른다고 보고했다. 텍스처가 없는 데다 빛까지 바뀌면 photometric consistency 가정이 동시에 무너진다. 실외에서 구름이 지나가거나, 실내에서 형광등이 깜빡이는 상황이 그 예다.
...
넷째, SuperPoint(2018)·LightGlue(2023) 같은 학습 기반 feature가 "feature는 정보를 버린다"는 direct method의 핵심 비판을 약화시켰다. 기존 handcrafted descriptor보다 훨씬 많은 정보를 보존하면서도 descriptor matching의 실용적 장점을 유지한다. direct method가 feature-based를 공격하던 그 지점에, learned feature가 자리를 메운 것이다.

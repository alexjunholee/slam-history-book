...
> 🔗 **차용.** Gauss-Newton photometric registration은 이미지 정렬 분야의 고전이다. Lucas-Kanade(1981) tracker와 그 역방향 합성(Baker & Matthews 2004)이 LSD-SLAM frontend의 직접 조상이다. Cremers 그룹은 variational image processing 커뮤니티의 언어를 SLAM 파이프라인 전체로 이식했다.
...
> 🔗 **차용.** Photometric camera calibration의 형식적 기반은 Debevec & Malik(1997)의 HDR 복원 작업에서 비롯된다. 그들이 여러 장의 사진에서 camera response function을 복원하기 위해 세운 photometric 모델을 DSO는 실시간 SLAM의 최적화 변수로 가져왔다.
...
2018년 Engel et al.은 DSO에 IMU를 결합한 [VI-DSO](https://doi.org/10.1109/CVPR.2018.00947)를 발표했다. 동기는 단순했다. photometric direct method의 가장 큰 실패 모드인 조명 급변 상황에서 IMU의 관성 측정이 pose 추적을 보조할 수 있다. 또한 mono 카메라의 scale ambiguity를 IMU로 해소할 수 있다.

VI-DSO는 DSO의 windowed photometric bundle adjustment에 IMU preintegration factor를 추가한다. IMU preintegration 방식은 Forster et al.의 2017년 논문에서 차용했다. 결과적으로 scale이 복원되고 극단적 조명 조건에서 robustness가 향상되었다.

Cremers 그룹의 후속 작업들, Basalt(2019)와 DM-VIO(2022)도 같은 방향을 이었다. direct photometric frontend에 tightly coupled inertial backend를 붙이는 구조다. 이 계보는 feature-based VIO(VINS-Mono, OpenVINS)와 병렬로 진행되면서 각자의 생태계를 형성했다.

> 🔗 **차용.** VI-DSO의 IMU preintegration은 [Forster et al. 2017. On-Manifold Preintegration (PAMI)](https://doi.org/10.1109/TPAMI.2016.2597517)의 manifold preintegration 공식을 그대로 사용한다. DSO의 photometric layer 위에 Forster의 inertial layer가 올라간 적층 구조다.
...
첫째, photometric calibration 의존성이다. DSO가 가정하는 vignetting 보정, response curve 보정, 노출 제어는 consumer camera에서 임의로 얻어지지 않는다. 스마트폰 카메라는 HDR 합성, auto-exposure, 실시간 화이트 밸런스를 자체적으로 적용하며, 그 파이프라인은 사용자에게 공개되지 않는다. DSO의 photometric 모델은 이런 카메라에서 기본 가정이 깨진다.

둘째, 조명 변화다. DSO 논문은 photometric gradient가 부족한 환경에서 추적 실패율이 급격히 오르는 것을 실험으로 보고했다. 이것은 텍스처 부재의 문제이기도 하지만, 조명이 바뀌는 순간 photometric consistency 가정이 무너지는 문제이기도 하다. 실외 환경에서 구름이 지나가거나, 실내에서 형광등이 깜빡이는 상황은 direct method에 즉각적 위협이다.

셋째, 벤치마크와 실제 배포 간의 간극이다. controlled dataset에서의 우월한 성능이 실제 로봇 배포로 이어지지 않았다. ORB-SLAM은 여러 카메라 모델에서 별도 calibration 없이 동작한다. 시스템 인테그레이터 입장에서 이 차이는 결정적이다.

넷째, learned feature의 등장이 direct method의 동기 일부를 흡수했다. SuperPoint(2018)·LightGlue(2023)처럼 학습 기반 feature는 기존 handcrafted descriptor보다 훨씬 많은 정보를 보존하면서도 descriptor matching의 실용적 장점을 유지한다. "feature는 정보를 버린다"는 direct method의 주된 비판이 약화된 것이다.
